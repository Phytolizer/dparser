...
extern struct D_ParseNode *my_ambiguity_fn(struct D_Parser *pp, int n, struct D_ParseNode **v);
...
int main(int argc, char *argv[]) {
  ...
  while ((ch = getopt(argc, argv, "vphI:")) != -1) switch (ch) {
      case 'v':
        d_verbose_level++;
        break;
      case 'p':
      ...
      case 'I':
        ...
        if (strlen(v_incdirs) + strlen(optarg) + 2 > sizeof(incdirsbuf)) exit(1);
  ...
  for (arg = optind; arg < argc; arg++) {
    ...
    if (dump_pp) {
      int i;
      for (i = 0; i < size; i++) putchar(filebuf[i]);
    }
