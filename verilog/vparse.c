...
static int getdeflen(const char *s, int srcleft) {
  ...
  if (!(s[0] == '_' || (s[0] >= 'a' && s[0] <= 'z') || (s[0] >= 'A' && s[0] <= 'Z')) || srcleft < 1) return 0;

  deflen = 1;

  while (srcleft > deflen && ((s[deflen] >= 'a' && s[deflen] <= 'z') || (s[deflen] >= 'A' && s[deflen] <= 'Z') ||
...
static int ifdef_directive(struct fileinfo_s *pfi) {
  ...
  pfi->ifdefd_out <<= 1;
  if (ifndef && defined || !ifndef && !defined) pfi->ifdefd_out |= 1;
...
static char *find_include(char *filename) {
  ...
  if (access(filename, R_OK) == 0) return filename;

  if (!v_incdirs) return NULL;

  cp1 = v_incdirs;
  while (cp1[0]) {
    cp2 = file2;
    while (cp1[0] && cp1[0] != ':' && (cp2 - file2) < sizeof(file2)) *cp2++ = *cp1++;
    if (cp1[0] == ':') cp1++;
    ...
    if (cp2 == file2) continue;

    *cp2++ = '/';
    strcpy(cp2, filename);

    if (access(file2, R_OK) == 0) return file2;
  }
...
static int include_directive(struct fileinfo_s *pfi) {
  ...
  while (pfi->srcleft > 0 && isspace(pfi->sp[0])) pfi->sp++, pfi->srcleft--;
  if (pfi->srcleft > 0 && pfi->sp[0] == '"') pfi->sp++, pfi->srcleft--;
  ...
  *cp++ = '\0';
  ...
  if (v_getfile2(filename, pfi) < 0) return -1;
      ...
      if (is_start_of_line) {
        ...
        for (i = 0; ppdirectives[i].name; i++) {
          ...
          if (pfi->srcleft >= l && (isspace(pfi->sp[l]) || pfi->sp[l] == '\n') &&
            ...
            if ((*ppdirectives[i].handler)(pfi) < 0) return -1;
...
#define HANDLE_C_COMMENTS
    ...
    if (pfi->sp[0] == '/' && pfi->srcleft > 1 && pfi->sp[1] == '*') {
      ...
      while (pfi->srcleft > 1 && (pfi->sp[0] != '*' || pfi->sp[1] != '/')) {
        if (*pfi->sp == '\n') f_appendc(pfi, '\n');
...
int v_getfile(const char *filename, char **pbuf, int *plen) {
  ...
  if (!fi.deftab) fprintf(stderr, "v_getfile: couldn't malloc deftab.\n"), exit(1);
 ...
 */
static void def_add(struct deftab_s *dt, const char *name, int namelen, char *substitution) {
  int h = deftab_hash(name, namelen);
  struct deftab_e_s **ppde = &dt->bucket[h];
  struct deftab_e_s *pde;

  while ((pde = *ppde)) {
    if (strncmp(name, pde->name, namelen) == 0 && strlen(pde->name) == namelen) {
  ...
  if (!pde) fprintf(stderr, "adddef: out of memory."), exit(1);
  *ppde = pde;

  pde->name = malloc(namelen + 1);
  if (!pde->name) fprintf(stderr, "addef: out of memory.\n"), exit(1);
...
static char *def_find(struct deftab_s *dt, const char *name, int namelen) {
  ...
  while (pde) {
    if (strncmp(name, pde->name, namelen) == 0 && strlen(pde->name) == namelen) return pde->substitution;
    pde = pde->next;
  }
  return NULL;
}
...
static char *keyword_table[] = {
    ...
    "edge",       "else",      "end",         "endattribute", "endcase",  "endfunction", "endmodule", "endprimitive",
    ...
    "rtranif1",   "scalared",  "signed",      "small",        "specify",  "specparam",   "strength",  "strong0",
...
int v_iskeyword(const char *s, const char *e) {
  ...
  for (i = 0; keyword_table[i]; i++)
    if (slen == keyword_length[i] && strncmp(s, keyword_table[i], slen) == 0) return 1;
...
void v_parse_init(void) {
  ...
  for (i = 0; keyword_table[i]; i++) keyword_length[i] = strlen(keyword_table[i]);
}
