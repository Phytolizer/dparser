...
#include "gramgram.hpp" // must appear before dparser.h
#include "d.hpp"
...
void add_global_code(Grammar *g, char *start, char *end, int line)
{
    if (!g->code)
    {
        g->code = (Code *)MALLOC(sizeof(Code) * 4);
    }
    else if (!((g->ncode + 1) & 4))
    {
...
D_Pass *find_pass(Grammar *g, char *start, char *end)
{
    ...
    {
        if (l == g->passes.v[i]->name_len && !strncmp(g->passes.v[i]->name, start, l))
        {
            return g->passes.v[i];
        }
    }
...
{
    char *n = (char *)(p ? p->name : " _synthetic");
...
static Term *lookup_token(Grammar *g, char *name, int l)
{
    ...
    for (i = 0; i < g->terminals.n; i++)
    {
        ...
        if (t->kind != TERM_TOKEN || t->string_len != l || strncmp(t->string, name, l))
        {
            continue;
        }
        return t;
        ...
        for (i = 0; i < g->states.n; i++)
        {
            ...
            for (j = i + 1; j < g->states.n; j++)
            {
                ...
                for (k = 0; k < s->reduce_actions.n; k++)
                {
                    ...
                    if (s->reduce_actions.v[k]->rule->prod != ss->reduce_actions.v[k]->rule->prod)
                    {
                        goto Lcontinue;
                    }
    ...
    for (i = 0; i < g->states.n; i++)
    {
        ...
        if (e->eq)
        {
            if (d_verbose_level > 2)
            {
                ...
                if (e->diff_state)
                {
                    printf("diff state (%d %d) ", e->diff_state->index, eq[e->eq->index].diff_state->index);
                }
                if (e->diff_rule)
                {
                    printf("diff rule ");
...
static void print_term_escaped(Term *t, int double_escaped)
{
    ...
    {
        char *quote = (char *)(double_escaped ? "\\\"" : "\"");
...
static void print_global_code(Grammar *g)
{
    ...
    for (i = 0; i < g->ncode; i++)
    {
        ...
        if (strstr(g->code[i].code, "<stdio.hpp>") != NULL)
        {
            print_stdio_h = 0;
        }
    }
