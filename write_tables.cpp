...
static void new_offset(File *fp, const char *name)
{
...
static OffsetEntry *search_for_offset(File *fp, const char *name)
{
...
static OffsetEntry *get_offset(File *fp, const char *name, ...)
{
    int n;
    char buf[256];
    va_list ap;
...
static char *make_name(const char *fmt, ...)
{
    int n;
    char *h_buf, buf[256];
    va_list ap;
...
static void print_no_comma(File *fp, const char *str)
{
    if (!fp->binary)
    {
        fprintf(fp->fp, "%s", str);
...
static void print(File *fp, const char *str)
{
    if (!fp->binary)
    {
        fprintf(fp->fp, "%s", str);
...
static void start_struct_fn(File *fp, int size, const char *type_str, const char *name, const char *whitespace)
{
    new_offset(fp, name);
    fp->first_member = 1;
    if (fp->binary)
...
static void start_array_fn(File *fp, int type_size, const char *type_prefix, const char *type_str, const char *name,
                           const char *length_str, int length_data, const char *whitespace)
{
    new_offset(fp, name);
    if (fp->binary)
    {
...
static void add_struct_ptr_member_fn(File *fp, void **dest, OffsetEntry *oe, const char *format)
{
    if (fp->binary)
    {
...
static void add_array_ptr_member_fn(File *fp, OffsetEntry *oe, const char *format, int last)
{
    if (fp->binary)
    {
        add_array_member_internal(fp);
...
static void add_array_member_fn(File *file, CopyFuncType copy, const char *format, uint data, int last)
{
    if (file->binary)
    {
        add_array_member_internal(file);
...
static void end_struct_fn(File *fp, int size, const char *whitespace)
{
    if (fp->binary)
    {
        fp->tables.cur += size;
...
static void end_struct_in_array(File *fp, const char *last)
{
    if (fp->binary)
    {
        fp->tables.cur += fp->elem_size;
...
static void end_array(File *fp, const char *whitespace)
{
    if (fp->binary)
    {
...
static const char *make_type(int i)
{
    switch (i)
    {
    case 1:
...
static const char *scanner_type(State *s)
{
    return make_type(scanner_size(s));
}

static const char *make_u_type(int i)
{
    switch (i)
    {
    case 1:
...
static const char *scanner_u_type(State *s)
{
    ...
    for (i = 0; i < g->states.n; i++)
    {
        ...
        for (j = 0; j < s->scanner.transitions.n; j++)
        {
            ...
            for (k = 0; k < va->n; k++)
            {
                ...
                {
                    add_array_ptr_member(fp, D_Shift *, "&",
                                         get_offset(fp, "d_shift_%d_%s", va->v[k]->term->index, tag), 0);
                }
                else
                {
                    add_array_ptr_member(fp, D_Shift *, "&",
                                         get_offset(fp, "d_tshift_%d_%s", va->v[k]->term->index, tag), 0);
                }
            }
        ...
        for (j = 0; j < ss->n; j++)
        {
            if (!s->same_shifts)
            {
                ...
                for (k = 0; k < g->scanner_blocks; k++)
                {
                    ...
                    vsblock[ivsblock].chars = (ScanState **)&ss->v[j]->chars[k * g->scanner_block_size];
                    ...
                    if (xv == yv)
                    {
                        ...
                        for (x = 0; x < g->scanner_block_size; x++)
                        {
                            ...
                            add_array_member_fn(fp, get_copy_func(size), "%d",
                                                ss->v[j]->chars[xx] ? ss->v[j]->chars[xx]->index + 1 : 0,
                    ...
                    if (s->scan_kind != D_SCAN_LONGEST || s->trailing_context)
                    {
                        ...
                        yv = (ScannerBlock *)set_add_fn(ptrans_scanner_block_hash, xv, &trans_scanner_block_fns);
                        if (xv == yv)
                        {
                            int size = scanner_size(s);
                            start_array_fn(fp, size, "", make_type(size),
                                           make_name("d_accepts_diff_%d_%d_%d_%s", i, j, k, tag), "SCANNER_BLOCK_SIZE",
                                           SCANNER_BLOCK_SIZE, "\n");
                            for (x = 0; x < g->scanner_block_size; x++)
                            {
                                xx = x + k * g->scanner_block_size;
                ...
                if (ss->v[j]->accepts.n)
                {
                    ...
                    for (k = 0; k < ss->v[j]->accepts.n; k++)
                    {
                        ...
                        {
                            add_array_ptr_member(fp, D_Shift *, "&",
                                                 get_offset(fp, "d_shift_%d_%s", a->term->index, tag), 0);
                        ...
                        {
                            add_array_ptr_member(fp, D_Shift *, "&",
                                                 get_offset(fp, "d_tshift_%d_%s", a->term->index, tag), 0);
                            if (k == ss->v[j]->accepts.n - 1)
                            {
                                add_array_ptr_member(fp, D_Shift *, "&", &null_entry, 1);
                                end_array(fp, "\n\n");
        ...
        if (ss->n && !s->same_shifts)
        {
            ...
            for (j = 0; j < ss->n; j++)
            {
                ...
                for (k = 0; k < g->scanner_blocks; k++)
                {
                    ...
                    vs.transitions = (ScanStateTransition **)&ss->v[j]->transition[k * g->scanner_block_size];
                    xv = &vs;
                    yv = (ScannerBlock *)set_add_fn(pscanner_block_hash, xv, &scanner_block_fns);
                    assert(yv != xv);
                    add_struct_ptr_member(fp, SB_uint8, "",
                                          get_offset(fp, "d_scanner_%d_%d_%d_%s", yv->state_index, yv->scanner_index,
                                                     yv->block_index, tag),
                                          scanner_block[k]);
                ...
                print(fp, "}");
                end_struct_in_array(fp, (char *)(j != ss->n - 1 ? ",\n" : "\n"));
                g->write_line += 1;
            }
            end_array(fp, "\n\n");
            g->write_line += 2;
                ...
                for (j = 0; j < ss->n; j++)
                {
                    ...
                    for (k = 0; k < g->scanner_blocks; k++)
                    {
                        ...
                        add_struct_ptr_member(fp, SB_trans_uint8, "",
                                              get_offset(fp, "d_accepts_diff_%d_%d_%d_%s", yv->state_index,
                                                         yv->scanner_index, yv->block_index, tag),
                                              scanner_block[k]);
                        if (k != g->scanner_blocks - 1)
                        {
                            if ((k % 2) == 1)
    ...
    for (i = 0; i < g->states.n; i++)
    {
        ...
        if (s->right_epsilon_hints.n)
        {
            ...
            for (j = 0; j < s->right_epsilon_hints.n; j++)
            {
                ...
                add_struct_member(fp, D_RightEpsilonHint, "%d", s->right_epsilon_hints.v[j]->state->index,
                                  preceeding_state);
                add_struct_ptr_member(
                    fp, D_RightEpsilonHint, "&",
                    get_offset(fp, "d_reduction_%d_%s", reduction_index(s->right_epsilon_hints.v[j]->rule), tag),
                    reduction);
                end_struct_in_array(fp, (char *)((j == s->right_epsilon_hints.n - 1) ? "" : ","));
            }
            end_array(fp, "\n");
            g->write_line += 1;
        }
...
static void write_reductions(File *file, Grammar *g, char *tag)
{
    ...
    if (pdefault)
    {
        ...
        {
            fprintf(fp, "int d_pass_code_%d_%d_%d_%s%s;\n", i, rdefault->prod->index, rdefault->index, tag,
                    reduction_args);
            g->write_line += 1;
        }
    }
    for (i = 0; i < (int)g->productions.n; i++)
    {
        ...
        for (j = p->rules.n - 1; j >= 0; j--)
        {
            ...
            for (k = 0; k < j; k++)
            {
                if (r->elems.n == p->rules.v[k]->elems.n &&
                    ...
                    r->op_priority == p->rules.v[k]->op_priority && r->op_assoc == p->rules.v[k]->op_assoc &&
                    r->rule_priority == p->rules.v[k]->rule_priority && r->rule_assoc == p->rules.v[k]->rule_assoc &&
        ...
        for (j = 0; j < (int)p->rules.n; j++)
        {
            ...
            {
                char fname[256];
                sprintf(fname, "int d_speculative_reduction_code_%d_%d_%s%s ", r->prod->index, r->index, tag,
                        reduction_args);
            ...
            {
                sprintf(speculative_code, "d_speculative_reduction_code_%d_%d_%s", rdefault->prod->index,
                        rdefault->index, tag);
            }
            else
            {
            ...
            {
                if (rdefault && (int)rdefault->pass_code.n > pmax)
                {
        ...
        for (i = 0; i < g->states.n; i++)
        {
            ...
            if (s->error_recovery_hints.n)
            {
                ...
                if (h == s)
                {
                    ...
                    print(fp, (char *)(s->error_recovery_hints.n > 1 ? "\n" : ""));
                    for (j = 0; j < s->error_recovery_hints.n; j++)
                    {
                        t = s->error_recovery_hints.v[j]
                                ->rule->elems.v[s->error_recovery_hints.v[j]->rule->elems.n - 1]
                                ->e.term;
                        ss = escape_string(t->string);
                        start_struct_in_array(fp);
                        add_struct_member(fp, D_ErrorRecoveryHint, "%d", s->error_recovery_hints.v[j]->depth, depth);
                        add_struct_member(fp, D_ErrorRecoveryHint, "%d",
                                          s->error_recovery_hints.v[j]->rule->prod->index, symbol);
                        add_struct_str_member(fp, D_ErrorRecoveryHint, ss, string);
                        end_struct_in_array(fp, (char *)(j == s->error_recovery_hints.n - 1 ? "" : ",\n"));
    ...
    if (g->states.n)
    {
        ...
        for (i = 0; i < g->states.n; i++)
        {
            ...
            if (s->scanner.states.n)
            {
                ...
                add_struct_ptr_member(fp, D_State, "", get_offset(fp, "d_scanner_%d_%s", shifts->index, tag),
                                      scanner_table);
            }
            else
            {
            ...
            end_struct_in_array(fp, (char *)(i == g->states.n - 1 ? "\n" : ",\n"));
        }
        end_array(fp, "\n\n");
    }
    else
            ...
            else
            {
                ...
                for (i = 0; i < g->terminals.n; i++)
                {
                    if (g->terminals.v[i]->kind == TERM_TOKEN)
                    {
                        ...
                        fprintf(hfp, "%s = %d%s", g->terminals.v[i]->string,
                                g->terminals.v[i]->index + g->productions.n, i == g->terminals.n - 1 ? "" : ", ");
                    }
                }
                fprintf(hfp, "\n};\n");
            }
...
static const char *d_symbol[] = {"D_SYMBOL_STRING", "D_SYMBOL_REGEX", "D_SYMBOL_CODE", "D_SYMBOL_TOKEN"};
...
static void write_passes(File *fp, Grammar *g, char *tag)
{
    ...
    if (g->passes.n)
    {
        ...
        for (i = 0; i < g->passes.n; i++)
        {
            ...
            end_struct_in_array(fp, (char *)(i < g->passes.n - 1 ? ", " : ""));
        }
        end_array(fp, "\n\n");
        g->write_line += 2;
    }
                                  ...
                                  uint *str_len)
{
    ...
    if (!binary)
    {
        ...
        fprintf(fp, "#include \"dparse.hpp\"\n");
        g->write_line++;

        if (header)
        {
            fprintf(fp, "#include \"%s.d_parser.hpp\"\n", base_pathname);
            g->write_line++;
        }
        fprintf(fp, "\n");
        g->write_line++;
